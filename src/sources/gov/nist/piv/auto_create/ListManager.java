...
/* loaded from: JPIV Test Data Generator.jar:gov/nist/piv/auto_create/ListManager.class */
public class ListManager extends DataManager {
  ...
  public static String ATTRIBUTE_MIN_LENGTH = "minLength";
  public static String ATTRIBUTE_MAX_LENGTH = "maxLength";
  public static String ATTRIBUTE_REMOVE_WHITESPACE = "removeWhitespace";
  ...
  public ListManager(Node node) throws DOMException {
    this();
    loadValues(node);
  }
  ...
  public void loadValues(Node node) throws DOMException {
    ...
    if (node == null || !node.getNodeName().equals(TAG_DATA_MANAGER)) {
      throw new RuntimeException("ListManager: ListManager not created with valid node");
    }
    HashMap<String, String> hm = XMLUtils.getAttributes(node);
    String dmType = hm.get(ATTRIBUTE_TYPE);
    if (!dmType.equals(ListManager.class.getName())) {
      throw new RuntimeException("ListManager: ListManager not created with valid node");
    }
    String name = hm.get(ATTRIBUTE_HEADER);
    setHeader(name);
    String minLength = hm.get(ATTRIBUTE_MIN_LENGTH);
    setMinLength(Integer.parseInt(minLength));
    String maxLength = hm.get(ATTRIBUTE_MAX_LENGTH);
    setMaxLength(Integer.parseInt(maxLength));
    String removeWhitespace = hm.get(ATTRIBUTE_REMOVE_WHITESPACE);
    setRemoveWhiteSpace(removeWhitespace != null && removeWhitespace.equals("true"));
  ...
  public void addElement(Document document, Element element, int level) throws DOMException {
    ...
    root.setAttribute(ATTRIBUTE_TYPE, ListManager.class.getName());
    root.setAttribute(ATTRIBUTE_HEADER, getHeader());
    root.setAttribute(ATTRIBUTE_MIN_LENGTH, String.valueOf(getMinLength()));
    root.setAttribute(ATTRIBUTE_MAX_LENGTH, String.valueOf(getMaxLength()));
    root.setAttribute(ATTRIBUTE_REMOVE_WHITESPACE, String.valueOf(getRemoveWhiteSpace()));
  ...
  public ListManager(String name, int minLength, int maxLength) {
    this(name);
  ...
  private void updateDescription() {
    String out;
    String out2 = this.description + " ";
    if (getMinLength() == getMaxLength()) {
      out = out2 + "Fixed Length:" + getMinLength();
    } else if (getMaxLength() == -1) {
      out = out2 + "Min Length:" + getMinLength();
    } else {
      out = out2 + "Length:" + getMinLength() + "-" + getMaxLength();
    }
    this.jLabelDescription.setText(out);
  }
  ...
  private JButton getJButtonAdd() {
    if (this.jButtonAdd == null) {
      ...
      this.jButtonAdd.addActionListener(
          new ActionListener() { // from class: gov.nist.piv.auto_create.ListManager.1
            public void actionPerformed(ActionEvent e) {
              ListManager.this.getJListAvailable();
              if (!ListManager.this.listModel.contains(ListManager.this.jTextFieldNew.getText())) {
                String value = ListManager.this.jTextFieldNew.getText();
                if (ListManager.this.getRemoveWhiteSpace()) {
  ...
  private JButton getJButtonDelete() {
    if (this.jButtonDelete == null) {
      ...
      this.jButtonDelete.addActionListener(
          new ActionListener() { // from class: gov.nist.piv.auto_create.ListManager.2
            public void actionPerformed(ActionEvent e) {
              JList list = ListManager.this.getJListAvailable();
              int[] selectedItems = list.getSelectedIndices();
              for (int i = selectedItems.length - 1; i >= 0; i--) {
                ListManager.this.listModel.remove(selectedItems[i]);
              }
            }
          });
    }
