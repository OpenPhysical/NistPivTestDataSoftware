...
/* loaded from: JPIV Test Data Generator.jar:com/tvec/utility/configuration/editor/edit_panel/cell_handlers/KeyCryptographicMechanismListEditor.class */
public class KeyCryptographicMechanismListEditor extends JPanel implements BaseHandler {
  ...
  public KeyCryptographicMechanismListEditor() {
    try {
      jbInit();
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  public String getValue() {
    return this.jTextFieldList.getText().trim();
  }

  private void jbInit() throws Exception {
    this.jButtonEdit.setMinimumSize(new Dimension(11, 20));
    this.jButtonEdit.setPreferredSize(new Dimension(30, 20));
    this.jButtonEdit.setToolTipText("Click to edit");
    this.jButtonEdit.setText("...");
    this.jButtonEdit.addActionListener(
        new KeyCryptographicMechanismListEditor_jButtonEdit_actionAdapter(this));
  ...
  public void jButtonEdit_actionPerformed(ActionEvent e) {
    String[] currentSelections = new String[0];
    if (getValue().length() > 0) {
      currentSelections = getValue().split(";");
    }
    DialogKeyCryptographicMechanismListEditor dialog =
        new DialogKeyCryptographicMechanismListEditor(currentSelections);
